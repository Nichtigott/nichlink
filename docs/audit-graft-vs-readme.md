...
| C33 | Post-release plugin/graft: "`overlay_static` allocates no plan but still performs contract, admission, and connector validation once." | `README.md:636`; `README.zh-CN.md:576` | **MET** | One `overlay_cuts` call (`overlay.rs:111-121`); validations as C18. "Once" is per overlay call; no caching exists. |
...
| C35 | Cost/binary claims: read-only topology has "No startup allocation"; the release plan has "no heap allocation, global constructor, inventory walk, or startup registration loop"; `overlay_static` "allocates no plan". | `README.md:617-621,631-637`; `README.zh-CN.md:560-563,573-577` | **UNVERIFIABLE by reading** | The code shape supports them (`release/release.rs:37-43,172-219` is `Copy` over slices; `renderer/pass.rs:39-101`; `overlay.rs:111-121`), but "no startup allocation" / "no global constructor" / "no `.inventory` linker section" are properties of the linked artifact. To verify: build the release binaries and run `tools/nichlink-release-audit` (it "rejects a remaining `.inventory` linker section", `README.md:612-613`), or inspect symbols/sections with `nm`/`readelf`; this read-only pass ran none of that. |
