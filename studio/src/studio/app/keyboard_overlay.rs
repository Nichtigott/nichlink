            ...
            Overlay::Graft(mut graft) => {
                ...
                if graft.editing {
                    ...
                    match key.code {
                        ...
                        KeyCode::Down => {
                            if graft.pane == 1 {
                                graft.plan_selected = (graft.plan_selected + 1)
                                    .min(graft.plans.len().saturating_sub(1));
                            } else {
                        ...
                        KeyCode::Char('o') => {
                            ...
                            if let Some(selector) = selector {
                                ...
                                self.event =
                                    "Graft: no plan exists for this selector yet".to_owned();
                            }
                            self.overlay = Some(Overlay::Graft(graft));
                            return;
                        }
