//! Self-contained const SHA-256 so identities can bake into compile-time
//! constants. Public only through the re-export in `super`; keep it that way.
//! 自包含的 const SHA-256，让身份可以烧进编译期常量。只通过 super 的
//! 重导出对外公开；请保持如此。

/// Compute SHA-256 over two slices, optionally separated by a zero byte.
/// 对两个字节片段计算 SHA-256，可选地在中间加入零字节。
pub(in crate::registry_core) const fn sha256_digest(
    first: &[u8],
    second: &[u8],
    separator: bool,
) -> [u8; 32] {
...
pub(in crate::registry_core) const fn sha256_prefix(
    first: &[u8],
    second: &[u8],
    separator: bool,
) -> [u8; 16] {
