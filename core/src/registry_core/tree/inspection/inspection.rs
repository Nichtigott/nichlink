...
impl Registry {
    ...
    /// Check one node's runtime checks against an observed value. The caller
    /// supplies the current call path so this method stays free of trace
    /// state; run_method passes `trace.current_path()`.
    /// 用观测值检查一个节点的运行期检查。由调用方提供当前调用路径，
    /// 使本方法不持有 trace 状态；run_method 侧传 `trace.current_path()`。
    pub fn health_check(
        &self,
        node: NodeId,
        value: &RuntimeValue,
        ...
        for check in &entry.info.runtime_checks {
            if let Err(failure) = check.run(value) {
                ...
                child.call_path = call_path.clone();
