//! Editor snippet generation for registration-face fields.
//! 注册面字段的编辑器 snippet 生成。
//!
//! Every editor's field completion inserts a bare field name, and a
//! language-server snippet does not fire inside a macro call's token tree, so
//! the `: ` after a field name only comes from the editor's own snippet layer —
//! which speaks a different format per editor.
//! 所有编辑器的字段补全都只插裸名字，而语言服务器 snippet 在宏调用的 token 树里
//! 不会触发，因此字段名后的 `: ` 只能来自编辑器自己的 snippet 层——而每个编辑器
//! 的格式不同。
...
const FIELD_SHAPES: &[(&str, &str)] = &[
    ...
    // The key is the kernel's field spelling; the shape is LSP snippet syntax,
    // which a const cannot be concatenated into (`concat!` takes literals only).
    // The test below keeps the two in step by asserting every shape starts with
    // its field name.
    // 键是内核的字段拼写；形状是 LSP 片段语法，常量无法拼进去（`concat!` 只接受
    // 字面量）。下面的测试通过"每个形状都以自己的字段名开头"把两者钉在一起。
    (lexicon::FACE_FIELD_PLUGIN, "plugin: $1,"),
...
impl Editor {
    /// Every editor this writer can inject snippets for, in CLI order.
    /// 本写入方能为哪些编辑器注入 snippet，按命令行顺序。
...
mod tests {
    use super::{
        Editor, FACE_FIELD_ORDER, FIELD_SHAPES, NVIM_SNIPPET_FILE, SNIPPET_FILE, editor_snippets,
        field_shape, luasnip_body, write_editor_snippets,
    };
    use std::fs;
