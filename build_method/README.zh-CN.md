...
必须有且仅有一条声明，其首个模块必须解析到包的源码根下的源文件——约定布局就是
`src/`（含 `src/bin/`），而 `[lib] path` 把库目标放到别处时，就是目标所在的
目录。损坏、重复或不可解析的入口会让构建失败并给出声明位置。
...
源码根跟随**库**目标。声明 `[lib] path = "host/lib.rs"` 的包，其注册面从
`host/` 读入，每个面的身份路径随之是 `host/…`——相对清单的路径，因为声明宏
只去掉一个前导 `src/`，别的都不去。`[lib] path` 指不到文件时是一条点名它的
`face-layout` 诊断，而不是去遍历别的树。`[[bin]]` 目标不移动源码根：一个包可能
有多个二进制目标，没有任何东西能在它们之间做选择。
