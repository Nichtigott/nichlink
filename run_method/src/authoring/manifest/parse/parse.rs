...
use crate::authoring::validation::{authoring_namespace, normalized_path, source_root};
use crate::{
...
fn parse_face_macro_impl(path: &Path, text: &str) -> Result<FaceManifest, String> {
    ...
    values.insert("source".to_owned(), source.clone());
    ...
    let parent_syntax = match face.parent() {
        Some(parent) => parent,
        None => directory_parent(&source)?,
    };
    match parent_syntax {
        ParentSyntax::Root => {
...
/// Derive the parent face from the folder tree when the declaration omits
/// `parent`: the nearest ancestor folder that carries a same-named face file
/// owns this face, and the package root owns faces without such an ancestor.
/// This mirrors the build crate's `directory_parent_id` so studio and the
/// build pipeline agree on the default mount point.
/// 声明省略 `parent` 时按目录推导父注册面：最近的、含有同名面文件的祖先
/// 文件夹即父级，否则挂到包根。与 build crate 的 `directory_parent_id`
/// 保持一致，保证 studio 和构建流水线对缺省挂载点理解相同。
fn directory_parent(source: &str) -> Result<ParentSyntax, String> {
    let root = source_root();
    let mut dir = Path::new(source)
        .parent()
        .map(Path::to_path_buf)
        .ok_or_else(|| "generated face has no module directory".to_owned())?;
    // The first candidate sits above the face's own folder.
    // 第一个候选在面自身文件夹的上一级。
    if !dir.pop() {
        return Ok(ParentSyntax::Root);
    }
    loop {
        ...
        if root.join(&candidate).is_file() {
            let parent_source = normalized_path(&candidate);
            let kind = kind_from_source_path(&parent_source)
                .ok_or_else(|| format!("cannot read the parent face {parent_source}"))?;
            return Ok(ParentSyntax::FromPath {
