...
macro_rules! __control_object {
    {
        ...
        $crate::__registration_face! {
            source: super::__REGISTRATION_SOURCE,
            collector: $collector,
            kind: $kind,
            preset: ($preset),
    ...
    // 精简写法按生成源码顺序排列；除 kind 外均可省略，并使用安全默认值。
    {
        ...
        $crate::__registration_face! {
            ...
            registry_name: $crate::__face_string_or!(super::__REGISTRATION_MODULE_NAME; [$($registry_name)?]),
            ...
            registry_rule_path: $crate::__face_expr_or!(super::__REGISTRATION_SOURCE; $($rule_path)?),
    ...
    // 最小可用注册面：kind 同时作为默认 handle 和注册槽位，末尾逗号可省略。
    {
        ...
        $crate::__registration_face! {
            ...
            registry_rule_path: super::__REGISTRATION_SOURCE,
